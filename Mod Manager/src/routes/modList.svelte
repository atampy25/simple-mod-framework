		...
		if (modFilePath.endsWith(".rpkg")) {
			...
			if (window.fs.readdirSync("./staging").every((a) => window.fs.existsSync(window.path.join("./staging", a, "manifest.json")))) {
				...
				if (
					...
					mergeConfig({ knownMods: [...getConfig().knownMods, json5.parse(String(window.fs.readFileSync(window.path.join("..", "Mods", window.fs.readdirSync("./staging")[0], "manifest.json")))).id] })
