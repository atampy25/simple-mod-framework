...
async function fetchUpdates() {
	...
	if (semver.lt(frameworkVersion, latestGithubRelease.tag_name)) {
		document.getElementById("frameworkUpdateAvailableText").innerHTML =
			{ patch: "Patch available", minor: "Minor update available", major: "Major update available" }[semver.diff(frameworkVersion, latestGithubRelease.tag_name)] || "Update available"
		document.getElementById("frameworkUpdateProcessText").innerHTML = latestGithubRelease.body.includes("CANNOT BE APPLIED AUTOMATICALLY")
			? "The update must be applied manually. Re-download the framework from the Nexus Mods page."
			: "The update can be applied automatically."
		...
		if (fs.existsSync(path.join("..", "Mods", modFolder, "manifest.json"))) {
			...
			if (modManifest.updateCheck) {
				...
				if (semver.lt(modManifest.version, modUpdateData.version)) {
					...
					$(
						"#modUpdateCards"
					)[0].innerHTML += `<div class="text-lg text-center p-4 m-4 shadow-2xl bg-gradient-to-br from-gray-800 to-gray-900 cursor-pointer" onclick="updateMod('${sanitiseStrongly(
						modFolder.replaceAll(`'`, "").replaceAll(`\\`, "")
					)}')">
	...
	try {
		if (!fs.existsSync(path.resolve("..", json5.parse(fs.readFileSync("../config.json")).runtimePath))) {
			showMessage(
				...
				"The framework can't find a folder it's looking for. Please re-read the installation instructions to ensure everything was installed correctly.",
				"error"
			)
			return
		}
		...
		if (
			!fs.existsSync(path.join(json5.parse(fs.readFileSync("../config.json")).retailPath, "Runtime", "chunk0.rpkg")) &&
			!fs.existsSync(path.join(path.resolve("..", json5.parse(fs.readFileSync("../config.json")).retailPath), "HITMAN3.exe"))
		) {
			showMessage(
				"Invalid Retail path",
				"The framework can't find HITMAN3.exe in the right place. Please re-read the installation instructions to ensure everything was installed correctly.",
				"error"
			)
			return
		}
		...
		if (
			fs.existsSync(path.join(json5.parse(fs.readFileSync("../config.json")).retailPath, "Runtime", "chunk0.rpkg")) &&
			!fs.existsSync(path.join(path.resolve("..", json5.parse(fs.readFileSync("../config.json")).retailPath), "..", "MicrosoftGame.Config"))
		) {
			showMessage(
				"Invalid Retail path",
				"The framework can't find the game config in the right place. Please re-read the installation instructions to ensure everything was installed correctly.",
				"error"
			)
			return
		}
		...
		showMessage(
			"Invalid Runtime path/files",
			"The framework can't find a folder it's looking for, or it can't see the files it needs. Please re-read the installation instructions to ensure everything was installed correctly.",
			"error"
		)
		return
	}
...
async function refreshMods() {
	...
	for (var i = config.loadOrder.length - 1; i >= 0; i--) {
		try {
			let modFolder = !(
				fs.existsSync(path.join("..", "Mods", config.loadOrder[i])) &&
				!fs.existsSync(path.join("..", "Mods", config.loadOrder[i], "manifest.json")) &&
				klaw(path.join("..", "Mods", config.loadOrder[i]))
					.filter((a) => a.stats.size > 0)
					.map((a) => a.path)
					.some((a) => a.endsWith(".rpkg"))
			) // Mod is not an RPKG mod
				? fs
						.readdirSync(path.join("..", "Mods"))
						.find(
							(a) =>
								fs.existsSync(path.join("..", "Mods", a, "manifest.json")) &&
								json5.parse(String(fs.readFileSync(path.join("..", "Mods", a, "manifest.json")))).id == config.loadOrder[i]
						) // Find mod by ID
	...
	$("#errorReportingText")[0].innerText = json5.parse(fs.readFileSync("../config.json")).reportErrors
		? "Error reporting is enabled (click here to change that). Your ID is " + json5.parse(fs.readFileSync("../config.json")).errorReportingID
		: "Error reporting is disabled (click here to change that)."

	if (fs.readdirSync("../Mods").length > 0) {
		for (let mod of config.loadOrder) {
			try {
				let modFolder = !(
					fs.existsSync(path.join("..", "Mods", mod)) &&
					!fs.existsSync(path.join("..", "Mods", mod, "manifest.json")) &&
					klaw(path.join("..", "Mods", mod))
						.filter((a) => a.stats.size > 0)
						.map((a) => a.path)
						.some((a) => a.endsWith(".rpkg"))
				) // Mod is not an RPKG mod
					? fs
							.readdirSync(path.join("..", "Mods"))
							.find((a) => fs.existsSync(path.join("..", "Mods", a, "manifest.json")) && json5.parse(String(fs.readFileSync(path.join("..", "Mods", a, "manifest.json")))).id == mod) // Find mod by ID
				...
				if (fs.existsSync(path.join("..", "Mods", modFolder, "manifest.json"))) {
					...
					$("#enabledMods")[0].innerHTML += `<div class="p-8 bg-gray-900 w-full flow-root shadow-xl rounded-md text-white">
															<div class="float-right">
																${
																	modManifest.options && modManifest.options.some((a) => a.type == "checkbox" || a.type == "select")
																		? `<neo-button small label="" gradientFrom="thisisjustsoitworkslmao" gradientTo="bg-gray-800" onclick="modSettings('${sanitiseStrongly(
																				modFolder.replaceAll(`'`, "").replaceAll(`\\`, "")
																...
																<neo-button small label="Disable" gradientFrom="from-rose-400" gradientTo="to-red-500" onclick="disableMod('${sanitiseStrongly(
																	modFolder.replaceAll(`'`, "").replaceAll(`\\`, "")
																)}')" style="display: inline">
																	<i class="fas fa-times" slot="icon"></i>
																</neo-button>
																<neo-button small label="Move" gradientFrom="from-fuchsia-400" gradientTo="to-violet-400" onclick="moveMod('${sanitiseStrongly(
																	modFolder.replaceAll(`'`, "").replaceAll(`\\`, "")
																)}')" style="display: inline">
															...
															<div class="float-left" style="max-width: ${window.visualViewport.height > 1080 ? "70%" : "60%"}">
																<div class="mb-2">
																	<h3 class="font-semibold text-xl inline"><img src="frameworkMod.png" class="w-8 inline align-middle">  <span class="align-middle">${sanitise(
																		modManifest.name
																	)} <span class="font-light">by ${modManifest.authors.map((a) => sanitise(a)).join(", ")}</span></span></h3><br>
																</div>
					...
					$("#enabledMods")[0].innerHTML += `<div class="p-8 bg-gray-900 w-full flow-root shadow-xl rounded-md text-white">
															...
															<div class="float-left" style="max-width: ${window.visualViewport.height > 1080 ? "70%" : "60%"}">
																<div class="mb-2">
																	<h3 class="font-semibold text-xl inline"><img src="rpkgMod.png" class="w-8 inline align-middle">  <span class="align-middle">${sanitiseStrongly(
																		modFolder.replaceAll(`"`, "").replaceAll(`\\`, "")
																	)}</span></h3><br>
																</div>
																<p>RPKG-only mod</p>
		...
		for (modFolder of fs
			...
			.filter(
				(folder) =>
					!config.loadOrder.includes(folder) &&
					(!fs.existsSync(path.join("..", "Mods", folder, "manifest.json")) ||
						!config.loadOrder.includes(json5.parse(String(fs.readFileSync(path.join("..", "Mods", folder, "manifest.json")))).id))
			)) {
			try {
				if (!fs.lstatSync(path.join("..", "Mods", modFolder)).isDirectory()) {
					if (path.extname(modFolder) == ".zip") {
						$("#availableMods")[0].innerHTML += `<div class="p-8 bg-gray-900 w-full flow-root shadow-xl rounded-md text-white">
																<div class="float-right">
																	<neo-button small label="Install" gradientFrom="from-teal-400" gradientTo="to-blue-500" onclick="importZIP('${path
																		.join("..", "Mods", sanitiseStrongly(modFolder.replaceAll(`'`, "").replaceAll(`\\`, "")))
																		.replaceAll(`\\`, `\\\\`)}', true)" style="display: inline">
																		<i class="fas fa-file-import" slot="icon"></i>
																	</neo-button>
																</div>
																<div class="float-left" style="max-width: 80%">
																	<div class="mb-2">
																		<h3 class="font-semibold text-xl inline"><img src="zipMod.png" class="w-8 inline align-middle">  <span class="align-middle">${sanitiseStrongly(
																			modFolder.replaceAll(`"`, "").replaceAll(`\\`, "")
																		)}</span></h3><br>
																	</div>
																	<p>Zipped mod</p>
						...
						$("#availableMods")[0].innerHTML += `<div class="p-8 bg-gray-900 w-full flow-root shadow-xl rounded-md text-white">
																...
																<div class="float-left" style="max-width: 80%">
																	<div class="mb-2">
																		<h3 class="font-semibold text-xl inline"><img src="invalidMod.png" class="w-8 inline align-middle">  <span class="align-middle">${sanitiseStrongly(
																			modFolder.replaceAll(`"`, "").replaceAll(`\\`, "")
																		)}</span></h3><br>
																	</div>
																	<p>Not a mod</p>
					...
					$("#availableMods")[0].innerHTML += `<div class="p-8 bg-gray-900 w-full flow-root shadow-xl rounded-md text-white">
															<div class="float-right">
																<neo-button small label="Enable" gradientFrom="from-emerald-400" gradientTo="to-lime-600" onclick="enableMod('${sanitiseStrongly(
																	modFolder.replaceAll(`'`, "").replaceAll(`\\`, "")
																)}')" style="display: inline">
...
async function enableMod(mod) {
	...
	if (
		fs.existsSync(path.join("..", "Mods", mod, "manifest.json")) &&
		json5.parse(String(fs.readFileSync(path.join("..", "Mods", mod, "manifest.json")))).options &&
		json5.parse(String(fs.readFileSync(path.join("..", "Mods", mod, "manifest.json")))).options.some((a) => a.type == "checkbox" || a.type == "select")
	) {
		modSettings(mod)
	}
}
...
async function disableMod(mod) {
	...
	config.loadOrder = config.loadOrder.filter(
		(a) => a != (fs.existsSync(path.join("..", "Mods", mod, "manifest.json")) ? json5.parse(String(fs.readFileSync(path.join("..", "Mods", mod, "manifest.json")))).id : mod)
	)
...
function checkModOptions(modFolder) {
	...
	for (var i = config.modOptions[manifest.id].length - 1; i >= 0; i--) {
		if (
			!manifest.options.some((a) => a.type == "checkbox" && a.name == config.modOptions[manifest.id][i]) &&
			!manifest.options.some((a) => a.type == "select" && a.group + ":" + a.name == config.modOptions[manifest.id][i])
		) {
			if (manifest.options.some((a) => a.type == "select" && a.name == config.modOptions[manifest.id][i])) {
				...
				config.modOptions[manifest.id][i] =
					manifest.options.find((a) => a.type == "select" && a.name == config.modOptions[manifest.id][i]).group +
					":" +
					manifest.options.find((a) => a.type == "select" && a.name == config.modOptions[manifest.id][i]).name
			} else {
	...
	for (var i = config.modOptions[manifest.id].length - 1; i >= 0; i--) {
		if (
			manifest.options.find((a) => (a.type == "checkbox" && a.name == config.modOptions[manifest.id][i]) || (a.type == "select" && a.group + ":" + a.name == config.modOptions[manifest.id][i]))
				?.requirements
		) {
			if (
				!manifest.options
...
async function modSettings(modFolder) {
	...
	for (let option of manifest.options) {
		if (option.type == "checkbox") {
			settingsHTMLs[column] += `<div class="mb-2"><label class="inline-flex items-center">
									<input${option.requirements && !option.requirements.every((a) => config.loadOrder.includes(a)) ? " disabled" : ""} type="checkbox"${
				json5.parse(fs.readFileSync("../config.json")).modOptions[manifest.id].includes(sanitiseStrongly(option.name.replaceAll(`"`, "").replaceAll(`\\`, ""))) ? " checked" : ""
			} class="form-checkbox cursor-pointer h-5 w-5 text-gray-700 bg-white" data-optionName="${sanitiseStrongly(option.name.replaceAll(`"`, "").replaceAll(`\\`, ""))}"><span class="ml-2${
				option.requirements && !option.requirements.every((a) => config.loadOrder.includes(a)) ? " text-gray-400" : ""
			}" data-optionName="${sanitiseStrongly(option.name.replaceAll(`"`, "").replaceAll(`\\`, ""))}">${sanitiseStrongly(option.name.replaceAll(`"`, "").replaceAll(`\\`, ""))}</span>
							</label></div>`
	...
	for (let group of Object.keys(groups)) {
		...
		for (let option of groups[group]) {
			settingsHTMLs[column] += `<br><label class="inline-flex items-center">
								<input${option[2] && !option[2].every((a) => config.loadOrder.includes(a)) ? " disabled" : ""} type="radio"${
				json5
					.parse(fs.readFileSync("../config.json"))
					.modOptions[manifest.id].includes(sanitiseStrongly(group.replaceAll(`"`, "").replaceAll(`\\`, "")) + ":" + sanitiseStrongly(option[0].replaceAll(`"`, "").replaceAll(`\\`, "")))
					? " checked"
					: ""
			} class="form-radio" name="${sanitiseStrongly(group.replaceAll(`"`, "").replaceAll(`\\`, ""))}" data-optionName="${
				sanitiseStrongly(group.replaceAll(`"`, "").replaceAll(`\\`, "")) + ":" + sanitiseStrongly(option[0].replaceAll(`"`, "").replaceAll(`\\`, ""))
			}">
								<span class="ml-2${option[2] && !option[2].every((a) => config.loadOrder.includes(a)) ? " text-gray-400" : ""}" data-optionName="${
				sanitiseStrongly(group.replaceAll(`"`, "").replaceAll(`\\`, "")) + ":" + sanitiseStrongly(option[0].replaceAll(`"`, "").replaceAll(`\\`, ""))
			}">${sanitiseStrongly(option[0].replaceAll(`"`, "").replaceAll(`\\`, ""))}</span>
							</label>`
		}
	...
	await Swal.fire({
		title: manifest.name,
		html: `${sanitise(manifest.description)}<br><br><div class="text-left mt-4 text-2xl font-semibold">Settings</div><div class="text-left overflow-auto h-96">${
			useColumns
				? `
			...
			for (let option of manifest.options.filter((a) => a.type == "checkbox")) {
				if (option.image || option.tooltip || (option.requirements && !option.requirements.every((a) => config.loadOrder.includes(a)))) {
					if (option.image && !option.image.includes('"') && !option.image.includes("..") && !option.image.includes(":")) {
						...
						document.querySelector(".swal2-container").insertAdjacentHTML(
							"beforeend",
							`
							<div tabindex="-1" role="dialog" aria-live="assertive" aria-modal="true" style="width: 50rem; display: grid; margin: auto; position: relative; box-sizing: border-box; flex-direction: column; justify-content: center; max-width: 100%; padding: 1.25em; border: none; border-radius: 5px; background: #19191a; font-family: inherit; font-size: 1rem; -webkit-tap-highlight-color: transparent; -webkit-animation: swal2-show 0.3s; animation: swal2-show 0.3s;">
								...
								<div class="swal2-html-container text-center" style="display: block;">${
									sanitise(option.tooltip) +
									(option.requirements && !option.requirements.every((a) => config.loadOrder.includes(a))
										? `<br>Requires: ${option.requirements.map((a) => sanitiseStrongly(a)).join(", ")}`
										: ``)
								}<br>
								</div>
							</div>
						...
						if (option[3] && !option[3].includes('"') && !option[3].includes("..") && !option[3].includes(":")) {
							...
							document.querySelector(".swal2-container").insertAdjacentHTML(
								...
								<div tabindex="-1" role="dialog" aria-live="assertive" aria-modal="true" style="width: 50rem; display: grid; margin: auto; position: relative; box-sizing: border-box; flex-direction: column; justify-content: center; max-width: 100%; padding: 1.25em; border: none; border-radius: 5px; background: #19191a; font-family: inherit; font-size: 1rem; -webkit-tap-highlight-color: transparent; -webkit-animation: swal2-show 0.3s; animation: swal2-show 0.3s;">
									...
									<div class="swal2-html-container text-center" style="display: block;">${
										sanitise(option[1]) +
										(option[2] && !option[2].every((a) => config.loadOrder.includes(a)) ? `<br>Requires: ${option[2].map((a) => sanitiseStrongly(a)).join(", ")}` : ``)
									}<br>
									</div>
								</div>
							`
		...
		didOpen: async () => {
			...
			setTimeout(() => {
							...
							let modFolder = !(
								...
								? fs
										...
										.find(
											(a) =>
												fs.existsSync(path.join("..", "Mods", a, "manifest.json")) &&
												json5.parse(String(fs.readFileSync(path.join("..", "Mods", a, "manifest.json")))).id == mod
										) // Find mod by ID
								: mod // Mod is an RPKG mod, use folder name
				...
				if (cycle < 100) {
					...
					showMessage(
						"Dependency cycle",
						"The framework couldn't sort your mods! Ask the developer of whichever mod you most recently installed to investigate this. Also, report this to Atampy26 on Hitman Forum or Discord.",
						"error"
					)
				}
